...
class PersonSample:
    ...
    #
    # It matters because plain `yaw` saturates: measured across 5,109 resolved
    # samples on 12_paper, 01_phone and 04_talking, 71% sit at the limit, the
    # median |yaw| is 1.08 and every percentile from p75 up is pinned at 1.50.
    # A slight glance and a full turn score the same, which is exactly the
    # misclassification reported from the annotated video. Shoulder width is a
    # far more stable denominator and does not pin when a subject sits in
    # profile -- which, on overhead CCTV, most of them do.
    #
